class Reaction < ApplicationRecord
  ...
  def replace_incompatible_types
    ...
    when "like"
      if (dislikes = reactions_to_reactable.select{ |reaction| reaction.reaction_type_id = type_ids[:dislike]} )
        likes.first.destroy
      end
      if (likes = self.class.user_reactable(self.user, self.reactable, :like))
